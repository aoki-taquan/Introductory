...
==== ⚠️ NAT Gateway のコスト事故に注意

NAT Gateway は *個人検証で最も事故になりやすいリソース*。東京リージョンでは1台あたり *約 \$0.062/時（月 \$45〜）+ データ処理 \$0.062/GB* が発生し、立てっぱなしだと *月に6,000〜7,000円以上* の固定費になる。VPC を削除してもサブネットや NAT Gateway が残っていれば課金は続く。

回避策：

- 検証終了後は必ず *NAT Gateway を削除* する（Elastic IP も合わせて解放）
...
- *パブリック IPv4*：2024年2月1日以降、*割り当て・アタッチの有無にかかわらず一律 \$0.005/時* が課金される（月約 \$3.6、Free Tier で12か月分は月750時間無料）。停止すると動的アドレスは変わる
...
S3 と DynamoDB だけが対象なのは、この2つが *歴史的経緯でリージョン単位のパブリックエンドポイント* を持ち、ルートテーブルベースの制御で十分賄えるため。他のサービスは後述のインタフェース型に統一されている。
...
- *時間課金 + データ処理課金*
- S3 もインタフェース型で作れるが、*同一リージョン内アクセスならゲートウェイ型を優先* する（無料）
