...
+ *MFA（多要素認証）を有効化*：仮想 MFA（Google Authenticator、1Password 等）またはハードウェアキー（YubiKey 等）
+ *アクセスキーを作らない*：ルートユーザーのアクセスキーは使わない、存在すれば削除する
+ *通知先メールを安全に管理*：乗っ取り時の復旧経路になる

MFA の設定は *IAM コンソール* の「セキュリティ認証情報」から行う。仮想 MFA アプリで QR コードを読み取り、6桁のコードを2回入力すれば有効化される。
...
*IAM Identity Center* は、AWS の公式なシングルサインオン／アイデンティティ基盤である。個人利用でも小規模利用でも、長期運用を見据えるなら *最初から Identity Center で運用する* のが現代的な選択である。
